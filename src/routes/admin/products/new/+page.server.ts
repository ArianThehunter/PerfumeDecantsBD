  ...
  createProduct: async ({ request, locals }) => {
    ...
    if (sizesJson) {
      try {
        ...
        return fail(400, { message: 'Invalid sizes format. Must be a valid JSON array.' });
      }
    }
    ...
    if (prodError || !newProduct) {
      ...
      return fail(500, { message: 'An internal database error occurred while creating the product. Please try again.' });
    }
    ...
    // Insert main image URL if provided/uploaded
    if (finalImageUrl) {
    ...
    logger.info('Product created', { productId: newProduct.id, name, brand });
